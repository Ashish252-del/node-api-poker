...
const getPoolGameHistoryByQuery = (query) => {
   return db.pool_game_history.findAll({ where: query, raw: true });
}

const getPoolGameTable = (query) => {
   return db.pool_game_tables.findOne({ where: query, raw: true });
}
...
const getUserDetails = async (user_id) => {
   let data = await sequelize.query(`SELECT username ,profile_image ,real_amount , practice_amount,bonus_amount ,coins,locked_amount ,win_amount  FROM users JOIN user_wallets ON users.user_id = user_wallets.user_id  where users.user_id = ${user_id}`
